...
def home():
    ...
    if logged_in_user:
        ...
        return template('message', message="Not logged in", logout_link=False)
...
def login(token):
    ...
    if logged_in_user:
        ...
        return template('message', message="Invalid token", logout_link=False)

@route('/logout')
def logout():
    response.delete_cookie(USERNAME, path=COOKIE_PATH)
    return template('message', message="Logged out", logout_link=False)
